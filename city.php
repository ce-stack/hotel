...
try {
    ...
    $soapHeaders[] =
        new \SoapHeader('http://www.w3.org/2005/08/addressing', 'Action', 'http://TekTravel/HotelBookingApi/DestinationCityList');

    $DestinationCityListRequest = new DestinationCityListRequest("EG" , true);

    $hotelService->__setSoapHeaders($soapHeaders);

    $cityList =  $hotelService->DestinationCityList($DestinationCityListRequest);
