...
function createClient(){
    try {
        $client = new TBOSOAPClient("http://api.tbotechnology.in/hotelapi_v7/hotelservice.svc?wsdl");
...
function getCityList($client){
    $DestinationCityListRequest = new DestinationCityListRequest('AE','SA');
...
function getHotels($client){
    ...
    $HotelSearchRequest = new HotelSearchRequest($CheckInDate, $CheckOutDate, $CountryName, $CityName, $CityId, $IsNearBySearchAllowed,
        $NoOfRooms, $GuestNationality, $RoomGuests, $PreferredCurrencyCode, $ResultCount, 'All', 'false', $ResponseTime);
